...
class ReposController extends GithubApiAppController {
    ...
    function index() {
        if (!empty($this->data)) {
            $data = $this->Github->find('search', array('repo' => $this->data['Repo']['name']));
    ...
    function browse($owner, $repo) {       
        $info = $this->Github->find('branches', array('owner' => $owner, 'repo' => $repo)); 
    ...
    function commits($owner, $repo) {
        $data = $this->Github->find('commits', array('owner' => $owner, 'repo' => $repo));
    ...
    function tree($owner, $repo, $sha) {
        $tree = $this->Github->find('tree', array('owner' => $owner, 'repo' => $repo, 'sha' => $sha));
        ...
        for ($i = 0; $i < count($tree['tree']); $i++):
            ...
            // last commit details
            $details = $this->Github->find('details', array('owner' => $owner, 'repo' => $repo, 'file' => $tree['tree'][$i]['name']));
    ...
    function subtree($owner, $repo, $sha, $folder) {
        $tree = $this->Github->find('file', array('owner' => $owner, 'repo' => $repo, 'sha' => $sha, 'file' => $folder));
        for ($i = 0; $i < count($tree['tree']); $i++):
            $data['tree'][$i]['name'] = $tree['tree'][$i]['name'];
            $data['tree'][$i]['sha'] = $tree['tree'][$i]['sha'];
            $data['tree'][$i]['type'] = $tree['tree'][$i]['type'];
    ...
    function blob($owner, $repo, $sha, $file) {
        $data = $this->Github->find('file', array(
